...
| **PHP** | Copies portable per-site PHP-FPM overrides (memory, timeouts, upload limits, PHP flags, and worker controls), validates the destination FPM configuration, and reloads that PHP version. Socket, site-user, log, chroot, and environment lines remain destination-owned. Pool values never enter the clone log or job state. | both |
