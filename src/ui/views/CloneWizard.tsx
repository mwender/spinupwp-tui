  ...
  useKeyboard((key) => {
    ...
    if (job.step === "clone" || job.step === "done") {
      ...
      if (verifyOpen != null) {
        ...
        // g — the dest lost its deploy script: copy the source's and open the
        // dest's Git settings, so re-entering it is one paste.
        if (name === "g" && drilled?.deployScriptLost && drilled.deployScript && drilled.destSiteId != null) {
          copyToClipboard(drilled.deployScript)
      ...
      <Panel title={title} active>
        <box style={{ flexDirection: "column", flexGrow: 1, paddingTop: 1 }}>
          {selected.map((s, i) => {
            ...
            const cur = i === idx
            const vmark = (s.verify ? (s.verify.ok ? " ✓verified" : " ✕mismatch") : s.verifying ? " verifying…" : "") + (s.deployScriptLost || s.cronError ? " · ⚠ see verify" : "")
            return (
        ...
        <box style={{ flexDirection: "column", flexGrow: 1, paddingTop: 1 }}>
          {site.verifying ? (
            ...
            <>
              ...
              {cloneNotes(site)}
              <text content={`v re-run${site.deployScriptLost ? " · g copy deploy script + open Git settings" : ""} · ← back to roster`} fg={theme.textFaint} wrapMode="none" />
            </>
          ) : (
  ...
  function hints() {
    ...
    if (job!.step === "clone" || job!.step === "done") {
      if (verifyOpen != null) {
        ...
        return [{ key: "v", label: "re-run" }, ...(drilled?.deployScriptLost ? [{ key: "g", label: "deploy script" }] : []), { key: "←", label: "back" }, { key: "esc", label: "close" }]
      }
      const done = selected.filter((s) => s.step === "done").length
      const errored = selected.filter((s) => s.step === "error").length
      const settled = selected.length > 0 && selected.every((s) => s.step === "done" || s.step === "error")
