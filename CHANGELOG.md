...
From 1.0.0, the public surface (CLI subcommands/flags, config file location and
format, and token-resolution behavior) changes incompatibly only in a major
version; such changes are called out here.
...
### Added
- **Install with Homebrew.** `brew install mwender/tap/spinuptui` installs SpinupTUI along with Bun, and `brew upgrade spinuptui` updates it. The About panel (`?`) recognizes a Homebrew install and shows `brew upgrade spinuptui` as its update command instead of the Bun one.
